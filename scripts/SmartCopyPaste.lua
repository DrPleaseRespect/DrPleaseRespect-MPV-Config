...
-- Copyright (c) 2025, DrPleaseRespect (Julian Nayr),
-- License: BSD 2-Clause License
-- Creator: Eisa AlAwadhi
-- Forked By: DrPleaseRespect (Julian Nayr)
...
--Changes are recommended to be made in the script-opts directory.
	...
	-----Script Settings----
	native_clipboard = true, --native clipboard utilizes mpv's native clipboard property, set to false if mpv is older than 0.40.0
...
function get_clipboard()
	local clipboard
	if o.native_clipboard == false then
...
function set_clipboard(text)
	local pipe
	local clipboard_status = false;
	if o.native_clipboard then 
		clipboard_status = mp.set_property("clipboard/text", text)
	end
	if clipboard_status == nil then
		if o.device == 'linux' then
